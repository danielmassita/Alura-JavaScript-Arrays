...
// DESAFIO - Ponto Extra - Um aluno recebeu um ponto extra nas suas notas, adicione esse ponto extra em suas notas em lista [10, 9, 8, 7, 6].

const notas = [10, 9, 8, 7, 6];

// for, forEach ou map.

var notasAtualizadas = notas.map(nota => nota == 10 ? nota : nota++); // Esse TERNÁRIO (tipo um for) vai retornar o valor de nota e somente DEPOIS somar 1. Nosso caso, precisamos INCREMENTAR +1 ANTES e depois retornar o valor da nota (atualizada).
console.log(notasAtualizadas);
console.log(notas);

console.log(' ');

notasAtualizadas = notas.map(nota => nota == 10 ? nota : ++nota);
console.log(notasAtualizadas);
console.log(notas);

// Diferença entre o .forEach() e o .map(): O primeiro método (.forEach()) ele não retorna nada, ele simplesmente EXECUTA o comando que foi imputado, que está dentro do bloco. Porém, esse método não retorna nada. POR OUTRO LADO, o segundo método (.map()), tudo que é feito como função/execução do call-back é RETORNADO como uma nova array, que é o que queremos, TEMOS uma array de notas e QUEREMOS uma array de notas atualizadas. O retorno de .map() vai retornar uma nova array, e teremos que salvar em uma nova variável. Métodos diferentes fazem coisas diferentes... :) 
